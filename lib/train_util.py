...
class TrainApp:
    def __init__(self,cuda,resolution,b_min,b_max,num_samples,use_octree):
        self.cuda = cuda
    ...
    def gen_sdf_to_file(self, netG, img, calib, sdf_save_path):
        image_tensor = img.to(device=self.cuda)
        calib_tensor = calib.to(device=self.cuda)

        netG.filter(image_tensor)
        coords, mat = create_grid(self.resolution, self.resolution, self.resolution, self.b_min, self.b_max)

        sdf = generate_sdf(netG,self.cuda,calib_tensor,self.num_samples,self.use_octree,coords,mat)
        np.save(sdf_save_path,sdf)

    def gen_surface_from_file(self, netG, netC, img, calib, sdf_load_path, save_path, num_views): 
        image_tensor = img.to(device=self.cuda)
        calib_tensor = calib.to(device=self.cuda)
        ...
        sdf = np.load(sdf_load_path)

        coords, mat = create_grid(self.resolution, self.resolution, self.resolution, self.b_min, self.b_max)
        verts, faces, _, _ = generate_surface(mat,sdf)

        # Now Getting colors
        interval = 2
        color = gen_color(netC, calib_tensor, self.cuda, verts, interval, num_views)

        save_obj_mesh_with_color(save_path, verts, faces, color)    

    def gen_mesh_color(self, netG, netC, img, calib, save_path, num_views):
        image_tensor = img.to(device=self.cuda)
        calib_tensor = calib.to(device=self.cuda)
        ...
        try:
            ...
            save_processed_img(image_tensor,save_img_path)
    ...
    def gen_mesh(self, netG, img, calib, save_path):
        image_tensor = img.to(device=self.cuda)
        calib_tensor = calib.to(device=self.cuda)
...
def save_processed_img(image_tensor,save_img_path):
    save_img_list = []
...
def gen_color(netC, calib_tensor, cuda, verts, interval, num_views):
    verts_tensor = torch.from_numpy(verts.T).unsqueeze(0).to(device=cuda).float()
    verts_tensor = reshape_sample_tensor(verts_tensor, num_views)
    color = np.zeros(verts.shape)

    for i in tqdm(range(len(color) // interval)):
        left = i * interval
